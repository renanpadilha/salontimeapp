...
app.get('/api/v1/clientes/:id_cliente/favoritos', function(req, res) {
	var id_cliente = req.params.id_cliente;
	knex.raw('SELECT f.id, f.favorito, e.nome, e.id as estabelecimento, e.rate FROM favoritos f JOIN estabelecimentos e ON f.id_estabelecimento = e.id WHERE f.id_cliente = ?', id_cliente)
...
app.get('/api/v1/servicos/:id/estabelecimentos/:id_estabelecimento', function(req, res, next) {
	var id_estabelecimento = req.params.id_estabelecimento;
	var id = req.params.id;
	knex.raw('SELECT s.*, es.id as idServico, es.preco FROM servicos s JOIN estabelecimentos_servicos es ON s.id = es.id_servico WHERE es.id_estabelecimento = ? AND es.id = ?', [id_estabelecimento, id])
	.then(function(servicos) {
		res.json(servicos.rows);
	}).catch(function(err) {
		console.log(err);
...
app.delete('/api/v1/servicos/:id/estabelecimentos/:id_estabelecimento', function(req, res) {
	var id_servico = req.params.id;
	var id_estabelecimento = req.params.id_estabelecimento;
	knex('estabelecimentos_servicos').where({id_servico: id_servico, id_estabelecimento: id_estabelecimento}).del()
	.then(function(response) {
		res.status(204).json();
	}).catch(function(error) {
		console.log(error);
...
app.delete('/api/v1/servicos/:id/estabelecimentos/:id_estabelecimento/profissionais', function(req, res) {
	var id_servico = req.params.id;
	var id_estabelecimento = req.params.id_estabelecimento;
	knex('profissionais_servicos').where({id_servico: id_servico, id_estabelecimento: id_estabelecimento}).del()
	.then(function(response) {
		res.status(204).json();
	}).catch(function(error) {
		console.log(error);
